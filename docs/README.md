# What to send Nuno

This folder is the DatabyPassion paperwork for the Real Life Technologies call. The website is separate: [https://reallifetechnologies.vercel.app](https://reallifetechnologies.vercel.app).

## Send after the call (English)

- [proposal-databypassion-reallife.pdf](proposal-databypassion-reallife.pdf) — cotação. Default **€9.500 + IVA**, Care Standard **€149/mês**.
- HTML source if he wants to read in a browser: [proposal-databypassion-reallife.html](proposal-databypassion-reallife.html)

Do not send during the call. Fill numbers only if Understand changed scope.

## Internal only (Portuguese) — do not send

- [nuno-call-script.pdf](nuno-call-script.pdf)
- [nuno-call-script.html](nuno-call-script.html)

On the call: Understand, show the prototype, promise the cotação the same day. Range if pushed: **8–15 mil euros**. Never say €9.500 out loud. No Jhonny, no Mercer, no EY.

## Rebuild PDFs

From the repo root, with Google Chrome installed:

```bash
npm run pdf
```
